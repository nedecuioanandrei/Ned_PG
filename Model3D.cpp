...
void Model3D::ReadOBJ(std::string fileName, std::string basePath) {
  ...
  for (size_t s = 0; s < shapes.size(); s++) {
    ...
    for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
      ...
      // gps::Texture currentTexture = LoadTexture("index1.png",
      // "ambientTexture"); textures.push_back(currentTexture);
    ...
    if (a > 0 && materials.size() > 0) {
      ...
      if (materialId != -1) {
        ...
        if (!ambientTexturePath.empty()) {
          ...
          currentTexture =
              LoadTexture(basePath + ambientTexturePath, "ambientTexture");
        ...
        if (!diffuseTexturePath.empty()) {
          gps::Texture currentTexture;
          currentTexture =
              LoadTexture(basePath + diffuseTexturePath, "diffuseTexture");
          textures.push_back(currentTexture);
        }
        ...
        if (!specularTexturePath.empty()) {
          gps::Texture currentTexture;
          currentTexture =
              LoadTexture(basePath + specularTexturePath, "specularTexture");
          textures.push_back(currentTexture);
        }
      }
    }
